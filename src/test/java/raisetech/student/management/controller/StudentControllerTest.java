...
class StudentControllerTest {
  ...
  void 受講生詳細の受講生の入力チェックで名前の文字数チェックで掛かること(){
    ...
    assertThat(violations).extracting("message").containsOnly("2文字以上50文字以下で入力してください");
  }

  @Test
  void 受講生詳細のコース情報の入力チェックに異常がないこと(){
    StudentCourses course = new StudentCourses(1,1,"Java", LocalDate.of(2025, 1, 1), LocalDate.of(2026, 1, 1));
  ...
  void 受講生詳細のコース情報の入力チェックのコース名の文字数チェックで掛かること(){
    StudentCourses course = new StudentCourses(1,1,"", LocalDate.of(2025, 1, 1), LocalDate.of(2026, 1, 1));

    Set<ConstraintViolation<StudentCourses>> violations = validator.validate(course);

    assertThat(violations.size()).isEqualTo(1);
    assertThat(violations).extracting("message").containsOnly("1文字以上20文字以下で入力してください");
  }
